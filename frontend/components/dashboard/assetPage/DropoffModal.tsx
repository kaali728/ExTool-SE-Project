...
export default function DropoffModal({
  ...
  useEffect(() => {
    if (open && selectedAsset !== null) {
      const foundDropOff = selectedAsset.table.find(
  ...
  const handleSubmit = async () => {
    ...
    const imagesDownloadUrl = await uploadImagesFiles(
      assetPictures,
      `assets/${selectedAsset?.name}/images/dropoff-${formData.date}/pictures/drop-off-${formData.date}`
    );

    const additionalDownloadUrls = await uploadAdditionalFiles(
      additionalPictureAcceptedFiles,
      `assets/${selectedAsset?.name}/images/dropoff-${
        formData.date
      }/additionalImages/drop-off-${formData.date}-${v4()}`
    );
    ...
    dispatch(
      updateSelectedAssetTable({
        tableContent: {
          ...
          status: ASSET_PICK_DROP.DROPEDOFF,
    ...
    >
      ...
      <Flex>
        ...
        <Input
          ...
          onChange={handleChange}
        />
        <Text margin="xl 0 m 0" scale="s" weight="bold">
          *Refuel before dropping off
        </Text>
        <Flex flexDirection="row" alignItems="center">
          <Input
            name="refuel"
            type={"number"}
            value={formData.refuel.toString()}
            onChange={handleChange}
          />
